...
- **Real-time Communication**: WebSocket bridge to Claude Agent Viewer daemon
...
- **Daemon**: Claude Agent Viewer daemon listening on Unix socket (`/tmp/claude-agent-viewer.sock`)
- **Bridge Process**: Converts Unix socket to WebSocket on `ws://localhost:8765`
- **Client**: WebSocket store (`wsStore.svelte.ts`) handles real-time communication
- **Response Format**: `{"type": "response", "data": {...}}` from daemon
- **Event Types**: init, pane_added, message, delta, status
- **Message Broadcasting**: Daemon broadcasts all messages automatically - no need to fetch history separately
- **Async Handling**: Fire-and-forget approach, rely on broadcast events

### WebSocket Commands (Full Suite)
- `panes` - List panes
- `status` - Get pane status
- `add` - Add pane (format: `{"action":"add","name":"<name>"}`)
...
- **WebSocket pane interface (chat-like)**:
  - Treat panes like chat conversations
  - Show message history (daemon broadcasts all messages automatically)
  ...
  - Activity sidebar showing live status/messages
...
- **Console logging for WebSocket**:
  ...
  - Log errors
  - Log messages
  - Prefix logs with `[ws]` for easy filtering
...
- Spawn agents in CAV environment
...
### WebSocket Integration (`wsStore.svelte.ts`)
- Connect to WebSocket bridge at `ws://localhost:8765`
- Handle daemon response format: `{"type": "response", "data": {...}}`
...
- Console logging for all WebSocket events ([ws] prefix)
- Process incoming event types: init, pane_added, message, delta, status
- Message history handled automatically via daemon broadcasts

### WebSocket UI Controls (Chat Interface)
- **Chat-like pane interface**:
  - Treat panes as chat conversations
  - Show full message history (broadcasted by daemon)
  ...
  - "Add agent..." input field for adding new panes
  ...
  - Add/remove pane controls
  - Message input for sending to panes
